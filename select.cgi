...
if [ ${FORM_field#had} != $FORM_field -o ${FORM_field#mohmat} != $FORM_field -o ${FORM_field#en4} != $FORM_field ]; then
  ...
  echo "Please consult their <a href="http://www.metoffice.gov.uk" target=\"_new\">website</a> for further information."
...
echo "Contains modified Copernicus Climate Change Service information."
  ...
  *) download=OK
  ...
  echo "Please consult the <a href=\"http://www-pcmdi.llnl.gov/ipcc/about_ipcc.php\" target=\"_new\">PCMDI website</a> for further information.<p>"
  ;;
  esac
elif [ "${FORM_field#rutgers}" != "${FORM_field}" ]; then
  echo "Rutgers University does not allow us to redistribute this file. "
  echo "Please contact <a href=\"http://climate.rutgers.edu/snowcover/docs.php?target=datareq\" target=\"_new\">Thomas Estilow</a> for access to these data."
elif [ "${FORM_field#tamsat}" != "${FORM_field}" ]; then
  echo "Reading University does not allow us to redistribute this file. "
  echo "Please contact <a href=\"http://www.met.reading.ac.uk/~tamsat/about/\" target=\"_new\">their site</a> for access to these data."
